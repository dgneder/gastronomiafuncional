...
const features = [
  "Bioquímica explicada (mecanismo de ação)",
  "Dosagem exata por idade e método",
  "Segurança: gestantes, crianças, pets",
  "Interações medicamentosas",
  "Receitas prontas com gotas exatas",
  "Guia de compra (como evitar fraudes)",
  "Investimento",
];
...
const columns = [
  {
    ...
    values: [
      ...
      { type: "no" },
      { type: "partial", note: "sem dosagem" },
      { type: "no" },
      { type: "text", text: "Grátis (com riscos)" },
    ],
  },
  {
    ...
    values: [
      ...
      { type: "yes" },
      { type: "yes" },
      { type: "partial", note: "nem sempre" },
      { type: "yes" },
      { type: "no" },
      { type: "text", text: "R$800–3.000" },
    ],
  },
  {
...
const ValueIcon = ({ type }: { type: string }) => {
  if (type === "yes") return <FaCheckCircle className="text-[#4A7C29]" />;
    ...
    <section className="py-16 sm:py-20 px-4 bg-white">
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 text-center mb-10 leading-tight">
          Compare e <span className="text-[#4A7C29]">decida você</span>
        </h2>
                ...
                {columns.map((col, i) => (
                  <th
                    ...
                    className={`p-3 text-center ${
                      col.highlight
                        ? "bg-[#F2F8ED] rounded-t-2xl border-2 border-b-0 border-[#4A7C29]"
                        : ""
                    }`}
                  >
                    <p
                      className={`font-black text-sm ${
                        col.highlight ? "text-[#2D5016]" : "text-gray-900"
                      }`}
                    >
                      {col.title}
                    </p>
                  ...
                  {columns.map((col, ci) => {
                    ...
                    return (
                      <td
                        ...
                        className={`p-3 text-center ${
                          col.highlight
                            ? "bg-[#F2F8ED] border-x-2 border-[#4A7C29]" +
