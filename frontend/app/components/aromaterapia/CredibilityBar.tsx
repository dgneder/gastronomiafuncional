"use client";

import React from "react";
import { FaGraduationCap, FaUniversity, FaBookOpen, FaMicroscope } from "react-icons/fa";

const stats = [
  {
    icon: <FaGraduationCap className="text-2xl text-[#4A7C29]" />,
    title: "Doutor em Genética",
    subtitle: "de Plantas",
  },
  {
    icon: <FaUniversity className="text-2xl text-[#4A7C29]" />,
    title: "Professor há 20+ anos",
    subtitle: "em universidade",
  },
  {
    icon: <FaBookOpen className="text-2xl text-[#4A7C29]" />,
    title: "160+ páginas",
    subtitle: "de conteúdo científico",
  },
  {
    icon: <FaMicroscope className="text-2xl text-[#4A7C29]" />,
    title: "Estudos publicados",
    subtitle: "como referência",
  },
];

const CredibilityBar: React.FC = () => {
  return (
    <section className="py-6 sm:py-8 px-4 bg-white border-y border-gray-100">
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => (
            <div key={index} className="text-center">
              <div className="flex justify-center mb-2">{stat.icon}</div>
              <p className="font-bold text-gray-900 text-sm sm:text-base">
                {stat.title}
              </p>
              <p className="text-gray-500 text-xs sm:text-sm">
                {stat.subtitle}
              </p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default CredibilityBar;
