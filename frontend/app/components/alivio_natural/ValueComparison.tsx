...
const rows = [
  ...
  {
    item: "Anti-inflamatórios (1 mês)",
    cost: "R$50–120",
    recurring: "Sim, todo mês",
    scientific: true,
    practical: true,
    safe: false,
  },
  {
    item: "Cursos e tratamentos alternativos",
    cost: "R$200–800",
    recurring: "Varia",
    scientific: false,
    practical: false,
  ...
  {
    item: "Pesquisa própria na internet",
    cost: "Tempo",
    recurring: "Horas",
    scientific: false,
    practical: false,
  ...
  {
    item: "Alívio Natural (guia completo)",
    cost: "R$27",
    recurring: "Nunca mais",
    scientific: true,
    practical: true,
    safe: true,
    highlight: true,
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-10">
          <p className="text-[#D97706] text-sm font-bold uppercase tracking-widest mb-2">
            Por que R$27 é uma decisão fácil
          </p>
              ...
              {rows.map(({ item, cost, recurring, scientific, practical, safe, highlight }) => (
                <tr
                  ...
                  className={highlight
                    ...
                    : "bg-white even:bg-[#FBF7EF] text-gray-700"
                  }
                >
                  <td className="p-4 font-semibold">
                    {highlight && <span className="text-[#D4A520] mr-2">⭐</span>}
                    {item}
                  </td>
                  <td className={`p-4 text-center font-bold ${highlight ? "text-[#D4A520]" : "text-[#1E2A10]"}`}>
                    {cost}
                  </td>
                  <td className="p-4 text-center text-xs">{recurring}</td>
                  <td className="p-4 text-center">
                    {scientific
                      ? <FaCheckCircle className={`mx-auto ${highlight ? "text-[#A3D977]" : "text-[#4A7C29]"}`} />
        ...
        <div className="text-center mt-8">
          <button
            ...
            className="inline-flex items-center gap-2 px-8 py-4 bg-[#D97706] hover:bg-[#B45309] text-white font-bold rounded-full shadow-lg transition-all hover:scale-[1.02]"
          >
            QUERO O ALÍVIO NATURAL POR R$27 →
          </button>
          <p className="text-xs text-gray-400 mt-2">
            Menos do que uma caixa de anti-inflamatório • Acesso imediato
          </p>
        </div>
      </div>
    </section>
