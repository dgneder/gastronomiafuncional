...
// ═══════════════════════════════════════════════════════════════
// CONFIGURAÇÃO — Altere estas constantes para seu produto
// ═══════════════════════════════════════════════════════════════
...
// Se não usar webhook (SellFlux/ActiveCampaign/etc), deixe vazio e
// o form vai redirecionar direto para o Hotmart com email pré-populado.
// ═══════════════════════════════════════════════════════════════
...
const Form: React.FC = () => {
  ...
  const buildCheckoutUrl = (email: string, name: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // If webhook configured, POST to it and let it redirect
    if (WEBHOOK_URL && WEBHOOK_URL !== "https://webhook.sellflux.com/webhook/v2/form/lead/SEU_WEBHOOK_ID_AQUI") {
      // Set the form action with redirect to Hotmart checkout
      const checkoutUrl = buildCheckoutUrl(email, name);
      ...
      // Direct redirect to Hotmart (no webhook)
      const checkoutUrl = buildCheckoutUrl(email, name);
