...
const DEFAULT_VIDEOS: VideoItem[] = [
  { mp4: "/panificacao/videos/frigideira.mp4",    poster: "/panificacao/gallery/frigideira.jpg",  title: "Pão de Frigideira",   subtitle: "10 minutos · SG · DIA" },
  { mp4: "/panificacao/videos/sourdough_corte.mp4", poster: "/panificacao/gallery/sourdough.jpg",  title: "Sourdough 48h",       subtitle: "Miolo alveolado · crosta" },
  { mp4: "/panificacao/videos/focaccia.mp4",       poster: "/panificacao/gallery/focaccia.jpg",   title: "Focaccia",            subtitle: "Alta hidratação · ANTI-INF" },
  { mp4: "/panificacao/videos/massa_sovada.mp4",   poster: "/panificacao/gallery/forma.jpg",      title: "A Massa",             subtitle: "Elasticidade sem glúten" },
  { mp4: "/panificacao/videos/hamburguer.mp4",     poster: "/panificacao/gallery/hamburguer.jpg", title: "Hambúrguer Funcional", subtitle: "IG 42 · SG" },
  { mp4: "/panificacao/videos/pizza.mp4",          poster: "/panificacao/gallery/pizza.jpg",      title: "Pizza Funcional",     subtitle: "Longa fermentação · SG" },
];
...
function VideoCard({ v }: { v: VideoItem }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    if (inView) { el.play().catch(() => {}); setPlaying(true); }
    else el.pause();
  }, [inView]);
  ...
  return (
    <section className="py-16 px-0 bg-white" data-aos="fade-up">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          <button
            aria-label="Anterior" onClick={() => scrollToIndex(active - 1)} disabled={active <= 0}
          ...
          <button
            aria-label="Próximo" onClick={() => scrollToIndex(active + 1)} disabled={active >= safeVideos.length - 1}
          ...
          <div className="mt-5 flex items-center justify-center gap-1.5">
            {safeVideos.map((_, i) => (
              <button
                key={i} aria-label={`Vídeo ${i + 1}`} onClick={() => scrollToIndex(i)}
                className="h-1.5 rounded-full transition-all duration-300"
                style={{ width: i === active ? "28px" : "6px", backgroundColor: i === active ? "#D4A04A" : "#D6D3D1" }}
              />
            ))}
          </div>
        </div>
