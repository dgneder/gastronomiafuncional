...
// ── VideoPlayer — setPlaying via onPlay, não dentro do useEffect ──────────────
...
function SplitLayout({ p, videoLeft }: { p: VideoSectionProps; videoLeft: boolean }) {
  return (
    <section
      ...
      style={{ background: videoLeft ? "#ffffff" : "linear-gradient(135deg, #FAF3E8, #F5E6CC)" }}
      data-aos="fade-up"
    >
      <div className="max-w-6xl mx-auto">
        <div className={`flex flex-col ${videoLeft ? "lg:flex-row" : "lg:flex-row-reverse"} items-center gap-12`}>
          ...
          <div className="w-full lg:w-1/2 space-y-6" data-aos={videoLeft ? "fade-left" : "fade-right"}>
            {p.tag && <p className="text-xs uppercase tracking-widest font-bold" style={{ color: "#C4622D" }}>{p.tag}</p>}
            ...
            {p.stat && (
              <div className="inline-flex items-baseline gap-2 px-5 py-3 rounded-xl border" style={{ backgroundColor: "#F5E6CC", borderColor: "#D4A04A40" }}>
                <span className="text-3xl font-extrabold" style={{ color: "#6B3A2A" }}>{p.stat.value}</span>
  ...
  return (
    <section className="relative py-0 overflow-hidden" data-aos="fade-up">
      <div className="relative w-full" style={{ minHeight: "420px" }}>
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(27,16,9,0.55) 0%, rgba(27,16,9,0.72) 60%, rgba(27,16,9,0.92) 100%)" }} />
...
function HighlightLayout({ p }: { p: VideoSectionProps }) {
  return (
    <section className="py-16 px-6 lg:px-12" style={{ background: "linear-gradient(180deg, #F5E6CC, #FAF3E8)" }} data-aos="fade-up">
      <div className="max-w-4xl mx-auto space-y-8 text-center">
        ...
        {p.ctaLabel && p.onCtaClick && (
          <button onClick={p.onCtaClick} className="group inline-flex items-center gap-3 px-10 py-4 text-base font-bold text-white rounded-xl shadow-xl hover:shadow-2xl hover:scale-[1.03] transition-all" style={{ background: "linear-gradient(135deg, #6B3A2A, #8B5A3C)" }}>
            {p.ctaLabel}
            <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
          </button>
        )}
