...
export type VideoSectionVariant =
  | "split-left"
  | "split-right"
  | "cinematic"
  | "highlight";
    ...
    <div ref={wrapRef} className={`relative overflow-hidden rounded-2xl shadow-2xl ${className}`}>
      ...
      {!playing && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div
            className="w-14 h-14 rounded-full flex items-center justify-center border-2 border-white/60"
    ...
    <section
      ...
      style={{
        background: videoLeft
          ? "#ffffff"
          : "linear-gradient(135deg, #FAF3E8, #F5E6CC)",
      }}
      data-aos="fade-up"
    >
      <div className="max-w-6xl mx-auto">
        <div className={`flex flex-col ${videoLeft ? "lg:flex-row" : "lg:flex-row-reverse"} items-center gap-12`}>
          ...
          <div className="w-full lg:w-1/2" data-aos={videoLeft ? "fade-right" : "fade-left"}>
            ...
            {p.caption && (
              <p className="text-stone-400 text-xs text-center mt-3 italic">{p.caption}</p>
            )}
          </div>

          <div className="w-full lg:w-1/2 space-y-6" data-aos={videoLeft ? "fade-left" : "fade-right"}>
            {p.tag && (
              <p className="text-xs uppercase tracking-widest font-bold" style={{ color: "#C4622D" }}>
                {p.tag}
              </p>
            )}
            ...
            {p.subtext && (
              <p className="text-lg text-stone-500 leading-relaxed">{p.subtext}</p>
            )}
            {p.stat && (
              <div
                className="inline-flex items-baseline gap-2 px-5 py-3 rounded-xl border"
                style={{ backgroundColor: "#F5E6CC", borderColor: "#D4A04A40" }}
              >
                <span className="text-3xl font-extrabold" style={{ color: "#6B3A2A" }}>
                  {p.stat.value}
                </span>
    ...
    <section className="relative py-0 overflow-hidden" data-aos="fade-up">
      <div className="relative w-full" style={{ minHeight: "420px" }}>
        ...
        <div
          ...
          style={{ background: "linear-gradient(180deg, rgba(27,16,9,0.55) 0%, rgba(27,16,9,0.72) 60%, rgba(27,16,9,0.92) 100%)" }}
        />
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 py-20 lg:py-28 min-h-[420px]">
          {p.tag && (
            <p className="text-amber-400/80 text-xs uppercase tracking-widest font-bold mb-4">{p.tag}</p>
          )}
          <h2 className="text-3xl lg:text-5xl font-extrabold text-white leading-tight max-w-3xl mb-6">
            {p.headline}
          </h2>
          {p.subtext && (
            <p className="text-amber-100/70 text-lg max-w-xl leading-relaxed mb-8">{p.subtext}</p>
          )}
          {p.ctaLabel && p.onCtaClick && (
            <button
              onClick={p.onCtaClick}
              className="group flex items-center gap-3 px-10 py-4 text-base font-bold rounded-xl shadow-2xl hover:scale-[1.04] transition-all text-stone-900"
          ...
          {p.caption && (
            <p className="text-white/30 text-xs mt-6 italic">{p.caption}</p>
          )}
        </div>
      </div>
    </section>
...
function HighlightLayout({ p }: { p: VideoSectionProps }) {
  return (
    <section
      className="py-16 px-6 lg:px-12"
      style={{ background: "linear-gradient(180deg, #F5E6CC, #FAF3E8)" }}
      data-aos="fade-up"
    >
      <div className="max-w-4xl mx-auto space-y-8 text-center">
        ...
        {p.subtext && (
          <p className="text-lg text-stone-500 leading-relaxed max-w-2xl mx-auto">{p.subtext}</p>
        )}
        <VideoPlayer src={p.videoSrc} poster={p.poster} className="w-full aspect-video" />
        {p.caption && (
          <p className="text-stone-400 text-xs italic">{p.caption}</p>
        )}
        {p.ctaLabel && p.onCtaClick && (
          <button
            onClick={p.onCtaClick}
            className="group inline-flex items-center gap-3 px-10 py-4 text-base font-bold text-white rounded-xl shadow-xl hover:shadow-2xl hover:scale-[1.03] transition-all"
            style={{ background: "linear-gradient(135deg, #6B3A2A, #8B5A3C)" }}
          >
            {p.ctaLabel}
            <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
